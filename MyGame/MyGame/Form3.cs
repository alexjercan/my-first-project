...
namespace WindowsFormsApplication2
{
    public partial class Form3 : Form
    {
        ...
        {
            if (player.Right >= Ban.Left && player.Left <= Ban.Right - player.Width / 2 && player.Bottom > Ban.Top && player.Top < Ban.Bottom && s==0)
            {
                s = 1;
                MessageBox.Show("Ai castigat un banut!");
                Ban.Hide();
                text.Hide();
            }

            if (player.Left <= Ban.Right && player.Right >= Ban.Left + player.Width / 2 && player.Bottom > Ban.Top && player.Top < Ban.Bottom && s == 0)
            {
                s = 1;
                Ban.Hide();
                text.Hide();
